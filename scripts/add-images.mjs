...
if (!GALLERY || !FROM) {
  ...
  Options
    ...
    --name=<base>      meaningful filename base (default: the gallery slug)

  Galleries:`);
  for (const kind of ["products", "applications", "projects"]) {
...
for (const k of ["products", "applications", "projects"]) {
...
// Continue the numeric series rather than restarting it. With --name the
// series runs under that meaningful base (docs/SOUL-PASS.md MOVE 6: names
// carry meaning); default stays the gallery slug.
const SERIES = BASE || GALLERY;
let nextN = 0;
for (const f of existing) {
  const m = f.match(new RegExp(`^${SERIES}-(\\d+)\\.`, "i"));
...
for (const file of incoming) {
  ...
  do {
    ...
    outName = `${SERIES}-${n}.jpg`;
    outPath = path.join(destDir, outName);
  } while (fs.readdirSync(destDir).some((f) => new RegExp(`^${SERIES}-${n}\\.`, "i").test(f)));
