 ...
 *
 * One band on stone (light, since 19 Sept — "too much dark mode"), one
 * composition, two ways in. No manufacturer named: the
...
export default function MaterialsBand() {
  ...
  return (
    <section className="relative overflow-hidden border-t border-hairline bg-surface-stone py-[7rem] max-[900px]:py-16">
      <div className="container-1280 relative z-[1] grid grid-cols-12 items-center gap-x-14 gap-y-14 max-[900px]:grid-cols-1">
        ...
        <div className="col-span-6 max-[900px]:col-span-1">
          <div className="relative aspect-[1800/1390] w-full" aria-hidden="true">
            {[c, b, a].map((sheet, i) => {
              ...
              return (
                <div
                  key={sheet.slug}
                  className="absolute inset-0 w-[84%] overflow-hidden rounded-[var(--radius)] bg-white shadow-[0_18px_44px_rgba(20,22,26,0.16)] ring-1 ring-black/[0.04]"
          ...
          <ul className="mt-9 flex flex-wrap gap-x-7 gap-y-2 text-[13px] text-ink-muted">
        ...
        <div className="col-span-6 max-[900px]:col-span-1">
          ...
          <h2 className="mt-5 max-w-[16ch] [text-wrap:balance]">Specified from a drawing, matched from a card</h2>
          <p className="mt-6 max-w-[48ch] text-[17px] leading-[1.65] text-ink-body [text-wrap:pretty]">
          ...
          <div className="mt-10">
            <div className="label">Eight of the colours</div>
              ...
              {STRIP.map((swatch) => (
                <li key={swatch.name}>
                  <Link href="/products/streetbond#colours" className="group block" title={`${swatch.name} · ${swatch.range}`}>
                    <span
                      aria-hidden="true"
                      className="block aspect-[1/1.15] rounded-[2px] ring-1 ring-black/10 transition-transform duration-200 group-hover:-translate-y-[2px]"
                      style={{ background: swatch.hex }}
                    />
                    <span className="mt-2 block text-[10.5px] leading-[1.3] tracking-[0.02em] text-ink-muted transition-colors group-hover:text-ink">
                      {swatch.name}
                    </span>
                  </Link>
                </li>
